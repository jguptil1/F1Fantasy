...
import constructor_predictions #FIXME: needs to create constructor_predictions
...
#### Loading data
def load_pre_race_constructor_features():
    with duckdb.connect(DATABASE_PATH) as con:
        result = con.execute("SELECT * FROM pre_race_constructor_features").df()
    return result

def load_current_week_constructor_features():
    with duckdb.connect(DATABASE_PATH) as con:
        result = con.execute("SELECT * FROM current_race_constructor_features").df()
    return result

def load_data():
    return load_pre_race_constructor_features(), load_current_week_constructor_features()
...
def train_and_evaluate(preprocess, models, X, y, X_train, X_test, y_train, y_test): 
    ...
    tscv = TimeSeriesSplit(n_splits=3) #smaller split size then driver model due to the lower row count
...
def predict_current_week(best_pipe, curr_df, feature_cols):
    ...
    #aligning current week cols to prediction cols
    X_curr = pd.DataFrame(
...
def save_model_artifacts(best_pipe, best_name, feature_cols, cv_mae):
    ...
    out_dir = Path("model_metadata/constructor")
    out_dir.mkdir(exist_ok=True)

    model_path = (
        out_dir /
        f"constructor_{best_name.lower()}_best_pipe.joblib"
    )

    meta_path = (
        out_dir /
        f"constructor_{best_name.lower()}_best_pipe_meta.json"
    )
...
def save_predictions(output_predictions, model_name, model_version, feature_set_version, target_variable, is_production_run, train_cutoff_race_id):
    ...
    constructor_predictions.append_constructor_run(prediction_run_row)

    #####CONSTRUCTOR PREDICTION ROWS APPEND OPERATION
    ...
    constructor_predictions.append_constructor_predictions(new_predictions_rows)
...
def run_constructor_model(model_name="v1_constructor", model_version="1", feature_set_version="1", target_variable="fantasy_points", is_production_run=False, run_tuning = False):
